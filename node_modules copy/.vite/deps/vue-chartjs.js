...
function getChartData(data, datasetIdKey) {
  ...
  setChartDatasets(nextData, __spreadValues({}, data), datasetIdKey);
  return nextData;
}
...
function setChartOptions(chart, options) {
  chart.options = __spreadValues({}, options);
}
  ...
  return defineComponent({
    ...
    setup(props, context) {
      ...
      function chartDataHandler(newValue, oldValue) {
        const newData = isProxy(newValue) ? toRaw(newValue) : __spreadValues({}, newValue);
        const oldData = isProxy(oldValue) ? toRaw(oldValue) : __spreadValues({}, oldValue);
